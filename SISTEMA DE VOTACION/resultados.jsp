<%@include file="WEB-INF/jspf/abstractfactory.jspf"%>
<%
    // Elegir fábrica según la plataforma
    UIFactory factory;
    
    if("movil".equals(plataforma)) {
        factory = new MovilFactory();  // Versión móvil
    } else {
        factory = new WebFactory();     // Versión web
    }
    
    // Crear componentes compatibles
    Boton boton = factory.crearBoton();
    Tabla tabla = factory.crearTabla();
    Notificacion noti = factory.crearNotificacion();
    
    // Datos de ejemplo
    Map<String, Integer> votos = new HashMap<>();
    votos.put("Candidato 1", 450);
    votos.put("Candidato 2", 300);
%>

<!-- Usar los componentes -->
<%= boton.renderizar("Ver detalles") %>
<%= tabla.renderizar(votos) %>
<%= noti.mostrar("Elecciones 2026") %>
