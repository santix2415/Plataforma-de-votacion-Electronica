...
<%@include file="WEB-INF/jspf/builder.jspf"%>
<%@include file="WEB-INF/jspf/factory.jspf"%>
<%
    ...
    if(request.getMethod().equals("POST")) {
        ...
        // Determinar nombre del candidato según el ID
        String nombreCandidato = "";
        if("1".equals(candidatoId)) nombreCandidato = "Candidato 1";
        else if("2".equals(candidatoId)) nombreCandidato = "Candidato 2";
        else if("3".equals(candidatoId)) nombreCandidato = "Candidato 3";
        else if("4".equals(candidatoId)) nombreCandidato = "Voto en Blanco";
        
       // Código patron builder
        Voto voto = new Voto.Builder()
            .conCedula(cedula)
            .conCandidato(nombreCandidato)
            .conFecha(new java.util.Date())
            .generarHash()
            .build();
        
        session.setAttribute("voto_realizado", true);
        session.setAttribute("candidato_votado", nombreCandidato);
        session.setAttribute("ultimoVoto", voto); 
    ...
    // Código del Factory Method (exportadores)
    Map<String, Integer> resultados = new HashMap<>();
    resultados.put("Candidato 1", 450);
    resultados.put("Candidato 2", 300);
  
    ExportadorService service = new ExportadorHTMLService();
    String reporteHTML = service.exportar(resultados);
    
    Exportador exportador = ExportadorFactory.crearExportador("TEXTO");
    String reporteTexto = exportador.exportarResultados(resultados);
    
    session.setAttribute("reporte", reporteHTML);
%>
