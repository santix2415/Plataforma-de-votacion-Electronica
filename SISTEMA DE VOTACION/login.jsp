<%@include file="WEB-INF/jspf/adapter.jspf"%>
<%
    if(request.getMethod().equals("POST")) {
        String cedula = request.getParameter("cedula");
        String password = request.getParameter("password");
        String tipoAuth = request.getParameter("tipoAuth");
        
        SistemaVotacionAuth auth = null;
        
        if("google".equals(tipoAuth)) {
            auth = (GoogleAuthAdapter) application.getAttribute("googleAuthAdapter");
        } else if("facebook".equals(tipoAuth)) {
            auth = (FacebookAuthAdapter) application.getAttribute("facebookAuthAdapter");
        } else {
            auth = (SistemaVotacionPropio) application.getAttribute("sistemaAuth");
        }
        
        if(auth.autenticar(cedula, password)) {
            session.setAttribute("usuario", cedula);
            session.setAttribute("nombre", auth.getNombreUsuario(cedula));
